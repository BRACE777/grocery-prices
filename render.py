...
FONT = (
    ...
    "family=Barlow+Condensed:wght@400;500;600;700&family=Barlow:wght@400;500;600;700"
...
:root{
  ...
  --flash:#ffd400; --flash-ink:#141414; --half:#c20d19;
  --stamp:#1b45c4; --focus:#1b45c4;
...
.masthead h1{
  font-family:"Barlow Condensed",sans-serif; font-weight:700; font-size:2.1rem;
...
.flash.half{background:var(--half); color:#fff}
.flash .chain{
...
.flash .now{
  font-family:"Barlow Condensed",sans-serif; font-weight:700; line-height:.85;
...
.flash .cut{font-weight:700; text-transform:uppercase; letter-spacing:.06em;
            font-size:.82rem}
.flash .rare{
  display:block; margin:.35rem 0 0; font-weight:700; font-size:.84rem;
...
.bay{
  ...
  background:var(--ink); color:var(--aisle);
...
/* ---------- a ticket ---------- */
.ticket{
  display:flex; flex-direction:column; min-height:10.6rem; position:relative;
...
.ticket .price{
  margin-top:auto; padding:0 .55rem; display:flex; align-items:baseline;
  gap:.08rem;
}
.ticket .price .cur{
...
.ticket .price .n{
  font-family:"Barlow Condensed",sans-serif; font-weight:700; font-size:2.9rem;
...
.ticket.best .unit{color:var(--chain-deep)}
.ticket .unit b{font-weight:700}

/* The special flash, a second card laid over the ticket's corner. */
.ticket .tag{
  position:absolute; top:-5px; right:0; background:var(--flash);
  color:var(--flash-ink); font-family:"Barlow Condensed",sans-serif;
  font-weight:700; font-size:.74rem; letter-spacing:.08em;
  text-transform:uppercase; padding:.12rem .38rem;
  transform-origin:top right;
}
.ticket .tag.half{background:var(--half); color:#fff}

/* The app's own mark, struck on the ticket rather than printed by the shop. */
.ticket .stamp{
  margin:0 .55rem .5rem; align-self:flex-start;
  font-family:"Barlow Condensed",sans-serif; font-weight:700; font-size:.72rem;
  letter-spacing:.09em; text-transform:uppercase;
  color:var(--stamp); box-shadow:inset 0 0 0 1.5px var(--stamp);
  padding:.08rem .3rem; transform:rotate(-1.4deg); transform-origin:left center;
}
...
.empty{
  min-height:10.6rem; display:flex; align-items:flex-end; padding:.55rem;
...
.fault{
  min-height:10.6rem; display:flex; flex-direction:column;
  justify-content:flex-end; gap:.2rem; padding:.55rem;
...
/* ---------- tally and footer ---------- */
.tally{
  margin:2.2rem 0 0; display:grid; grid-template-columns:1fr 1fr; gap:.45rem;
}
.tally div{
  background:var(--card); color:var(--card-ink); padding:.5rem .6rem .6rem;
  border-top:5px solid var(--chain);
}
.tally div[data-chain=woolworths]{--chain:var(--wool)}
.tally div[data-chain=coles]{--chain:var(--coles)}
.tally b{
  font-family:"Barlow Condensed",sans-serif; font-weight:700; font-size:2rem;
  line-height:1; display:block;
}
.tally span{font-size:.82rem; color:var(--card-soft);
...
@media (prefers-reduced-motion:no-preference){
  ...
  @starting-style{
    .ticket .stamp{transform:rotate(-1.4deg) scale(1.5); opacity:0}
  }
  ...
  @supports (animation-timeline:view()){
    ...
    @keyframes sign{
      from{background:var(--rail); color:var(--ink); box-shadow:none}
      to{background:var(--ink); color:var(--aisle);
         box-shadow:0 2px 0 var(--rail-lip)}
    }
  }
}
...
def _ticket(row: dict, chain: str, index: int) -> str:
    ...
    if offer is None:
        return (
            f'<div class="empty" style="--i:{index}">{label} '
    ...
    return (
        ...
        f'<span class="name">{e(offer["name"] or row["title"])}{size}</span>'
        f"{stamp}"
        f'<span class="price"><span class="cur">$</span>'
        f'<span class="n">{money(offer["price"])}</span></span>'
        f"{was}"
        f'<span class="unit">{unit}</span></a>'
    )
...
def _flash(entry: dict, lead: bool, position: int) -> str:
    ...
    half = " half" if "1/2" in text or "half" in text else ""
    ...
    cut = (
        f'<span class="cut">{offer["savingPercent"]}% off</span>'
        if offer.get("savingPercent")
        else ""
    )
    return (
        f'<a class="{klass}{half}" style="--j:{position}" '
        f'href="{e(offer["url"])}" rel="noreferrer">'
        ...
        f"{was}{cut}</span>"
        f'<span class="rare">{e(entry["phrase"])}</span></a>'
    )
...
def page(payload: dict) -> str:
    ...
    for chain in CHAINS:
        parts.append(
            f'<div data-chain="{chain}"><b>{wins[chain]}</b>'
            f"<span>lines cheaper at {CHAIN_LABEL[chain]}</span></div>"
        )
    parts.append("</div>")
